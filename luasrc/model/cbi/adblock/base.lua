...
if nixio.fs.access('/tmp/adblock/3rd/3rd.conf') then
    ...
    o.description = string.format(translate('AdGuardHome / Host / DNSMASQ / Domain rules auto-convert') ..
                                      '<br/><strong>' .. translate('Last Update Checked') .. ':</strong> %s<br/>', UD)
end
...
if tmp_rule == 1 then
    ...
    o.description = translate(
        'Delete 3rd rules files and delete the subscription link<br/>There is no need to click for modify the subscription link,The script will automatically replace the old rule file')
    o.write = function()
        SYS.exec('[ -d /tmp/adblock/3rd ] && rm -rf /tmp/adblock/3rd')
        SYS.exec(
            "grep -wq 'list url' /etc/config/adblock && sed -i '/list url/d' /etc/config/adblock && /etc/init.d/adblock restart 2>&1 &")
